...
## Delivery:        2018.06.26
## Script:          09_apportion-ghost-zip_proc.R
# #############################
...
#names(apportRule.df)
ghostZIPapport_idx <- which(apportRule.df$SharedBorderPrct==100)  # ZIP apport file index
...
apportRule_lst <- as.numeric(as.vector(apportRule.df$SharedBorderPrct[-ghostZIPapport_idx]))/100  # sum(apportRule)  # chk_09.02


# check that neighboring ZIP codes are inside NYC's 5 boroughs
neighborZIP <- c() ; neighborZIPborough <- c() ; apportRule <- c()
for (nn in neighbor_lst) {
    if (nn %in% boroughZIP$ZIP) {
        neighborZIP <- c(neighborZIP,nn)
        neighborZIPborough <- c(neighborZIPborough,boroughZIP[boroughZIP$ZIP==nn,2])
        apportRule <- c(apportRule,apportRule_lst[which(neighbor_lst==nn)])
    }
}
