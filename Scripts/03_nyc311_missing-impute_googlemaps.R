...
## Script:      03_nyc311_missing-impute_googlemaps.R
## Author:      Cedric Bhihe
...
} # format URL encoding for Google API query

geoCodeF <- function(address,latlng,verbose=FALSE,Nquery,
                     bounds=NULL,key=my_APIkey,language="en",region=NULL,components=NULL,simplify=F,curl_proxy=NULL) {
    ...
    if ( latlng != "" ) {
        ...
        doc <- getURL(u)
        # if ( as.integer(grep("OVER_QUERY_LIMIT",doc )) != 0 ) { 
        #     cat("\n\n****************\nGoogle daily query limit reached!\n****************\nAbort.\n")
        #     exit() 
        # }
        ...
        u <- urlF(address,latlng="")
        doc <- getURL(u)
        # if ( as.numeric(grep("OVER_QUERY_LIMIT", doc)) != 0 ) { 
        #     cat("\n\n****************\nGoogle daily query limit reached!\n****************\nAbort.\n")
        #     exit() 
        # }
        Nquery <- Nquery +1
    ...
    if(x$status=="OK") {
        ...
        fail_token=1; return(c("","","",fail_token,as.numeric(Nquery))) 
    }
}
...
#   "10048" for the former World Trade Center twin towers
...
proc_file <- paste0("Data/",
                      yearNbr,
                      ifelse(monthNbr<10,paste0("0",as.character(monthNbr)),as.character(monthNbr)),
                      "00_nyc311_proc")

# test to determine whether this is an initial run or a the continuation of one
# cat("Initial (i) or continued (c) post-processing of ZIP codes ? ")
reply <- readline(prompt = "Initial (i) or continued (c) post-processing of ZIP codes ? ")
...
if (reply == "i") {
        ...
        source_file <- paste0(proc_file,"02.csv")
        ...
        missingZIP_aggr$tabcomb[nrow(missingZIP_aggr$tabcomb),]  # view the "all-missing" section,  i.e. missing all, but for object + date of service request call 
        ...
        # if "-" exists in ZIP code, then trim from "-" to end of string, else keep ZIP unchanged
        ...
        Nna <- 0
        for (rr in 1:nrow(protoY)) {
            if (sum(is.na(protoY[rr,])) != 0 ) {
                Nna <- Nna +sum(is.na(protoY[rr,]))
                protoY[rr,is.na(protoY[rr,])] <- ""
            }
        }
        cat("\nNumber of suppressed 'NA' in base data: ",Nna,"\n")
    } else if (reply == "c") {
            source_file <- paste0(proc_file,"03-part.csv")
            protoY <- read.csv(source_file,
                               header=T,
                               sep=",",
                               quote="\"",
        ...
        cat("\n\nPlease, enter either 'i' or 'c'. Abort.")
        exit()
    }
...
if (! geolocalF) {
    ...
    ## vvvvvvvvvvvvvvv
    ## METHOD OFFICIALLY DEPRECATED BY GOOGLE CLOUD since 2018.06.11
    ## ^^^^^^^^^^^^^^^
    # Minimal https querying method to Google Maps' servers with package ggmap's method revgeocode()
    {  
        library("ggmap")  # load functions: revgeocode(), geocode()
        ...
        while (ii < length(checkZIP_idx)) {
            ...
            if (GPS_lst[zz] != "" 
                ...
                if (as.character(protoY$Xstreet_1[zz]) != ""){
                    searchAddress <- paste0(as.character(protoY$Address[zz])," + ",as.character(protoY$Xstreet_1[zz]))
                } else if (as.character(protoY$Intersect_1[zz]) != "") {
                    searchAddress <- paste0(as.character(protoY$Address[zz])," + ",as.character(protoY$Intersect_1[zz]))
                } # Some directions' address are incomplete but can be completed by either Xstreet of Intersect 
                
                while (is.na(gM_out[1]) & Nfails <= 20) {
                    Nfails <- Nfails +1
                    gM_out <- geocode(paste0(searchAddress,", New York, NY, USA"),
                                      output="latlona",
                                      override_limit=T)
                    Sys.sleep(APIsleep)
                }
                if (! is.na(gM_out[1])) {
                    ...
                    if (res_zip != "-1") {
                       ...
                       & token_Xstreet != 1) {
                searchAddress <- paste0(as.character(protoY$Xstreet_1[zz])," + ",as.character(protoY$Xstreet_2[zz]))
                while (is.na(gM_out[1]) & Nfails <= 20) {
                    Nfails <- Nfails +1
                    gM_out <- geocode(paste0(searchAddress,", New York, NY, USA"),
                                      output="latlona",
                ...
                searchAddress <- paste0(as.character(protoY$Intersect_1[zz]),
                                        " + ",
                                        as.character(protoY$Intersect_2[zz]))
                ...
                if (! is.na(gM_out[1])) {
                    ...
                    if (res_zip != "-1") {
                       ...
                       & as.character(protoY$Intersect_1[zz]) != ""
                       & token_crossintersect != 1) {
                # Some directions are erroneously logged as Xstreet_1 and Intersect_1
                
                searchAddress <- paste0(as.character(protoY$Xstreet_1[zz])," + ",as.character(protoY$Intersect_1[zz]))
                while (is.na(gM_out[1]) & Nfails <= 20) {
                    Nfails <- Nfails +1
                    gM_out <- geocode(paste0(searchAddress,", New York, NY, USA"),
                                      output="latlona",
  ...
  # Google Maps' API's querying with billing enabled:  USD 0.005/HTTP-GET
    {
        ...
        while (ii < length(checkZIP_idx)) {
            ...
            #if (as.numeric(Nquery) >= 0.98 * max_query){ breakFlag=1;break }
            
            if (GPS_lst[zz] != "" & token_GPS != 1) {
                # find position of comma in record of the form:  (__,__)
                sep_loc <- as.numeric(gregexpr(pattern =',',GPS_lst[zz])) 
                GPS_coord <- paste0(substr(GPS_lst[zz],2,sep_loc-1),",",substr(GPS_lst[zz],sep_loc+1,nchar(GPS_lst[zz])-1))
                gM_out=geoCodeF(address="",latlng=GPS_coord,verbose=FALSE,as.numeric(Nquery))
                ...
                if (gM_out[4] != "1") {
                    # Success in returning ZIP  
                    cat(paste(" zip:",gM_out[1],"\n"))
                    ZIP[zz] <- gM_out[1]
                    token_address <- 0
                    token_Xstreet <- 0
                    token_intersect <- 0
            ...
            } else if (as.character(protoY$Address[zz]) != "" & token_address != 1) {
                ...
                if (as.character(protoY$Xstreet_1[zz]) != ""){
                    ...
                    searchAddress <- as.character(protoY$Address[zz])
                } # Some directions' address are incomplete but can be completed by either Xstreet of Intersect
                
                gM_out=geoCodeF(paste0(searchAddress,", New York, NY, USA"),
                                latlng="",verbose=FALSE,as.numeric(Nquery))
                Nquery <- gM_out[5]
                ...
                searchAddress <- paste0(street_A," + ",street_B,", New York, NY, USA")
                
                gM_out=geoCodeF(searchAddress,latlng="",verbose=FALSE,as.numeric(Nquery))
                Nquery <- as.numeric(gM_out[5])
                ...
                searchAddress <- paste0(street_A," + ",street_B,", New York, NY, USA")
                gM_out=geoCodeF(address=searchAddress,latlng="",verbose=FALSE,as.numeric(Nquery))
                Nquery <- as.numeric(gM_out[5])
...
protoY <- cbind(ZIP=unlist(ZIP,use.name=F),protoY[,c(2:11)],GPS=unlist(GPS_lst,use.name=F))
length(which(! protoY$ZIP %in% c(ZIP_lst,"99999"))) 


# #############################
## Save processed csv file 
#   - either as `*_proc03.csv` (for complete processing)
#   - or as `*_proc03-part.csv` (for partial processing)
# #############################

if (breakFlag == 1) {
    old_target_file <- paste0(proc_file,"03-part.csv") 
    csvSaveF(protoY,old_target_file)     # overwrite csv to disk
} else {
    protoY <- protoY[which(protoY$ZIP %in% c(ZIP_lst,"99999")),]  # eliminate all foreign ZIP codes 
    ...
    if (file.exists(paste0(proc_file,"03-part.csv")) & file.exists(new_target_file)) {
        file.remove(paste0(proc_file,"03-part.csv"))
    }
}
