...
## Script:      12_nyc311-w-crime-data-mca
## Author:      Cedric Bhihe
...
mcaPlot1 <- plot.MCA(mcaNYC311,
                    ...
                    autoLab="yes",
                    cex=0.9,
                    col.ind = ccolors[as.numeric(Boroughs)], col.var = c(rep("lightsalmon3",52),rep("purple",12)),
                    col.ind.sup = "red", col.quanti.sup = "orchid3",
                    label = c("var","ind.sup","quanti.sup")
)
mcaPlot1 + legend("topright",legend=levels(Boroughs),cex=0.8,pt.cex=1.3,pch=16,col=ccolors[1:length(levels(Boroughs))])
