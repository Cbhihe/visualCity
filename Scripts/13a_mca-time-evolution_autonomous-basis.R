...
## Script:      13a_mca-time-evolution_autonomous-basis.R
## Author:      Cedric Bhihe
...
mcaCoords <- rbind(mcaNYC311$ind$coord,mcaNYC311$ind.sup$coord)
...
# Calculate change by Euclidian distance between 2010, 2014 and 2018
ncol(mcaCoords)
yearOfReference <- substr(periodOfReference,nchar(periodOfReference)-3,nchar(periodOfReference))
monthOfReference <- substr(as.Date(paste("01",periodOfReference),"%d %b %Y"),6,7) 

for (zip in substr(rownames(mcaCoords[grep(paste0("_",yearOfReference,monthOfReference),
                                           rownames(mcaCoords),value=F,fixed=T),]),1,5) ) {
    for (ff in 1:length(periodsOfInterest) ) {
